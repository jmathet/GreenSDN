...
def getFlowStat(topo, r):
    ...
    for p in range(0,k): # For each pod
        ...
        for j in range(0,density): # For each edge switch in the pod p 
            ...
            LEdge_up_p_e = (sum(rateUP))/(2.0**30)*8*2 # Total rate up in Gbits/sec TODO: ajouter math.ceil
            LEdge_down_p_e = (sum(rateDOWN))/(2.0**30)*8*2 # Total rate down in Gbits/sec TODO: ajouter math.ceil
