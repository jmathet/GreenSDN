...
def getFlowStat(topo, r):
    ...
    for p in range(0,k): # For each pod
        ...
        for j in range(0,density): # For each edge switch in the pod p 
            ...
            LEdge_up_p_e = math.ceil(sum(rateUP)/(2.0**30)*8*2/r) # Total rate up in Gbits/sec 
            LEdge_down_p_e = math.ceil(sum(rateDOWN)/(2.0**30)*8*2/r) # Total rate down in Gbits/sec
        ...
        for j in range(0,density): # For each aggregation switch in the pod p 
            ...
            LAgg_up_p = LAgg_up_p + sum(rateUP)/(2.0**30)*8*2/r # Total rate up in Gbits/sec
            LAgg_down_p = LAgg_down_p + sum(rateDOWN)/(2.0**30)*8*2/r # Total rate down in Gbits/sec
